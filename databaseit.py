...
def create_sample_database():
    ...
    df = pd.DataFrame(
        {
            'ba.com': ['UP', 'UP', 3, 4, 5, 6],
            'media.chrissy.org': ['UP', 5, 4, 3, 2, 1],
            'conservatory-main.chrissy.org': ['UP', 'b', 'c', 'a', 'a', 'b'],
        }
    )
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            dcc.Graph(id='graph'), className='ten columns'
                        ),
                    ],
...
def dff_to_table(dff_json, dropdown_x, dropdown_y):
    dff = pd.read_json(dff_json)
    return {
        'data': [{'x': dff[dropdown_x], 'y': dff[dropdown_y], 'type': 'bar'}],
        'layout': {'margin': {'l': 20, 'r': 10, 'b': 60, 't': 10}},
    }
